...
def calculate_r_node_co_degree_optimized(hyperedges, N, r):
    """
    优化版：计算r-node共度分布，避免显式生成所有r-node组合。
    只计算出现在超边中的r-node组合的共度。
    """
    # 使用字典记录每个r-node组合的共度
...
# 2. 计算理论临界阈值的函数 (Eq.6)
# ============================
def theoretical_critical_point(mean_kr, mean_kr_sq_minus_kr, m_fixed, r, p_H_fixed):
    """
    直接从矩计算理论临界节点保留概率 p_N_c。
    避免使用完整的分布数组。
    """
    # 计算C(m, r) = 组合数 C(m, r)
    ...
    # 公式(6)左侧等于1时临界，因此：
    # (p_N_c)**r * p_H_fixed * m_term * ratio = 1
    product = p_H_fixed * m_term * ratio
    if product <= 0:
        return None

    p_N_c_theory = (1.0 / product) ** (1.0 / r)

    # 确保结果在合理范围内
    if p_N_c_theory > 1.0:
        return 1.0
    elif p_N_c_theory < 0.0:
        return 0.0
...
# 4. 使用正确连接规则的蒙特卡洛模拟
# ============================
def monte_carlo_percolation_corrected(hyperedges, N, r, p_N, p_H, n_trials=10):
    """
    修正版蒙特卡洛模拟，使用正确的连接规则：
    两条超边共享至少r个节点则相连。
    """
    ...
    for trial in range(n_trials):
        # 步骤1: 随机损伤
        # 生成存活的节点
        node_survives = np.random.rand(N) < p_N
        alive_nodes = set(np.where(node_survives)[0])

        # 随机选择保留的超边，并检查其中的节点是否都存活
...
def main():
    ...
    m_fixed = 4  # 超边固定基数
    r = 2  # r-node的r值
    p_H_fixed = 0.8  # 固定的超边保留概率

    # 设置目标平均r-node共度λ_r
    # 对于r=2, m=4，使用合理的密度
    target_lambda_r = 0.36
    ...
    print(f"Hyperedge retention probability p_H = {p_H_fixed}")
    ...
    print(f"Total possible r-node combinations: C({N},{r}) = {C_N_r:,}")
    ...
    # 计算r-node共度统计量（优化版）
    print("Calculating r-node co-degree statistics (optimized method)...")
    result = calculate_r_node_co_degree_optimized(hyperedges, N, r)
    ...
    print(f"  Active r-node groups found: {len(kr_dict):,} (out of {total_r_nodes:,} possible)")
    print(f"  Mean co-degree <k_r> = {mean_kr:.6f}")
    print(f"  <k_r(k_r-1)> = {mean_kr_sq_minus_kr:.6f}")

    # 计算理论临界点 p_N_c
    print("\nCalculating theoretical critical threshold...")
    p_N_c_theory = theoretical_critical_point(mean_kr, mean_kr_sq_minus_kr, m_fixed, r, p_H_fixed)

    if p_N_c_theory is None:
        print("Failed to compute theoretical critical point.")
        return

    print(f"Theoretical critical p_N_c = {p_N_c_theory:.6f}")

    # 根据理论值设置p_N扫描范围
    #if p_N_c_theory > 0.9 or p_N_c_theory < 0.1:
    #    p_N_min, p_N_max = 0.1, 0.9
    #else:
        # 以理论值为中心，扩展±0.2的范围
    p_N_min = 0 #max(0.05, p_N_c_theory - 0.2)
    p_N_max = 1 #min(0.95, p_N_c_theory + 0.2)

    # 设置扫描点：每隔0.05取一个点
    n_points = 11 #int((p_N_max - p_N_min) / 0.05) + 1
    p_N_values = np.linspace(p_N_min, p_N_max, n_points)

    print(f"\nScanning p_N from {p_N_min:.3f} to {p_N_max:.3f} ({n_points} points)...")
    ...
    for i, p_N in enumerate(p_N_values):
        print(f"  Progress: {i + 1}/{len(p_N_values)} - p_N = {p_N:.3f}...", end="")
        R_mean, R_std = monte_carlo_percolation_corrected(hyperedges, N, r, p_N, p_H_fixed, n_trials=n_trials)
    ...
    if sim_critical_idx is not None and sim_critical_idx > 0:
        # 线性插值以获得更精确的估计
        p_low = p_N_values[sim_critical_idx - 1]
        p_high = p_N_values[sim_critical_idx]
        R_low = R_means[sim_critical_idx - 1]
        R_high = R_means[sim_critical_idx]

        if abs(R_high - R_low) > 1e-12:
            p_N_c_sim = p_low + (threshold_R - R_low) * (p_high - p_low) / (R_high - R_low)
        else:
            p_N_c_sim = p_high

        print(f"\nSimulated critical p_N_c (R > {threshold_R}) ≈ {p_N_c_sim:.6f}")

        if p_N_c_theory is not None:
            error_percent = abs(p_N_c_sim - p_N_c_theory) / p_N_c_theory * 100
            print(f"Relative error to theory: {error_percent:.2f}%")
    else:
        print(f"\nNo clear percolation transition observed in the scanned p_N range (R < {threshold_R}).")
        print("Try adjusting the p_N range or increasing network density.")
        p_N_c_sim = None
    ...
    # 绘制R vs p_N曲线
    plt.errorbar(p_N_values, R_means, yerr=R_stds, fmt='o-', capsize=5,
                 linewidth=2, markersize=6,
                 label=f'Monte Carlo simulation (N={N:,}, M={M:,})', color='royalblue')

    # 标记理论临界点
    if p_N_c_theory is not None:
        plt.axvline(x=p_N_c_theory, color='red', linestyle='--', linewidth=2,
                    label=f'Theoretical: $p_{{N_c}}$ = {p_N_c_theory:.4f}')

    # 标记模拟估计的临界点
    if p_N_c_sim is not None:
        plt.axvline(x=p_N_c_sim, color='green', linestyle='-.', linewidth=2,
                    label=f'Simulated: $p_{{N_c}}$ = {p_N_c_sim:.4f}')

    plt.xlabel('$p_N$', fontsize=14) #Node Retention Probability
    plt.ylabel('$R$', fontsize=14) #r-node Group Giant Component

    # 主标题
    #main_title = f'r-node Group Percolation (Fixed $p_H$={p_H_fixed})'
    #plt.title(main_title, fontsize=16, pad=20)
    #plt.title(f'r-node Group Percolation on Static Hypergraph (r={r}, m={m_fixed}, fixed $p_H$={p_H_fixed})',
    #          fontsize=15)

    # 参数信息
    param_text = f'$r$={r}, $m$={m_fixed}, fixed $p_H$={p_H_fixed}'
    ...
    plt.ylim(-0.05, 1.05)
    plt.xlim(-0.05, 1.05)
    #plt.xlim(min(p_N_values) - 0.02, max(p_N_values) + 0.02)
    plt.tight_layout()
    # 显示理论公式
    ...
    print(f"  p_H × m_term × (<k_r(k_r-1)>/<k_r>) = {p_H_fixed} × {m_term} × {ratio:.6f} = {product:.6f}")
    print(f"  (p_N_c)^r = 1 / {product:.6f} = {1 / product:.6f}")
    print(f"  p_N_c = ({1 / product:.6f})^(1/{r}) = {p_N_c_theory:.6f}")


# 运行主程序
if __name__ == "__main__":
    print("Corrected r-node percolation simulation (fixed p_H, scan p_N)")
    print("=" * 60)
    ...
    print("  2. More reasonable percolation threshold: R > 0.1 (instead of 0.005)")
    print("  3. Optimized implementation using inverted index for faster connection checking")
    print("=" * 60)
